...
3. **Atomic claim before every send.** A conditional `UPDATE … WHERE id AND <time_col> = original RETURNING id` row-locks the candidate so only one scheduler tick (across any number of replicas) wins it, and pushes the time column 10 minutes out so an in-flight send can't be re-claimed. This is the core duplicate-send guard.
...
6. **Mandatory test flight.** A campaign should never go live without a single test send first. (Enforced in the UI, not the API — see §10.3.)
...
| `EMAIL_MODE` (`config.emailMode`) | sender facade, bounce/reply | `gmail` / `smtp` / `brevo` / `mock`. Selects the **default** sender path. An `email_accounts` row with `auth_type='smtp'` always overrides to nodemailer regardless of this. Gmail bounce + Gmail reply pollers only run when `gmail`. |
...
| `EMAIL_MIN_DELAY` / `EMAIL_MAX_DELAY` | config | Present in config; the schedulers actually use a hardcoded 2000ms inter-send pause + the schedule-engine's per-day gaps. |
...
- **Auth:** OAuth2 refresh-token flow via `googleapis`. Two factories: `getGmailClient()` (singleton for the env account, from `GOOGLE_CLIENT_*`), and `createGmailClientFromCredentials(id, secret, refreshToken)` (per-DB-account, no singleton). `app_password` accounts are **also** routed through this Gmail-OAuth factory (they populate the `gmail_*` columns) — not through SMTP.
...
- **Global `rateLimiter` singleton** (env-account only): in-memory rolling hourly/daily counters (reset on restart, intentional) + a DB-persisted warmup ramp (`email_warmup_state`). Hardcoded global ramp: days 1–3 → 10/day, 4–7 → 20, 8–14 → 30, 15–21 → 40, 22+ → `EMAIL_DAILY_CAP`. `canSend()`, `recordSend()`, `waitUntilCanSend()`.
...
- [ ] Create the tables in §3 (`campaigns`, `campaign_leads`, `campaign_steps`, `email_accounts`, `email_warmup_state`, `warmup_emails`, `lead_notes` + its two unique indexes, `discovered_contacts`, your recipient/`leads` table).
