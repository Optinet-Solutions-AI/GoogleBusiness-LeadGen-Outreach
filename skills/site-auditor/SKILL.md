...
| `all_stock_photos` | Zero `<img>` tags using `lh3.googleusercontent.com/places/` AND ≥4 using `images.unsplash.com` | medium | Legitimate for leads with no scraped photos. For leads with real photos in DB, indicates the Vision fallback dropped them — check `vision.failed` in Cloud Run logs and confirm `web/lib/services/photo-selector.ts` is the post-`588b0c0` real-hash-fallback version. |
| `variant_homogeneity_with_neighbors` | DB lookup: this lead's variants share ≥4 of 8 slots (hero/services/reviews/trust/about/service_area/cta/service_detail_offset) with the most-recent neighbor lead | medium | enforceDiversity should have swapped at least one variant. Check the Cloud Run log line `stage_3.diversity.enforced`; verify fallback chains in `web/lib/picker.ts` aren't empty for the slot in question. |
| `hero_to_next_dead_zone` | **DOM check (desktop)**: pixel gap between hero `<section>` bottom and the next major block top exceeds 180px at 1280×900 | medium | The hero variant has an outlier bottom padding (e.g. animated-gradient was `pb-44` until `d8c4eb8`). Aligned heroes use `pb-20 md:pb-28`. Also check the next section's `pt-*` + any `border-y` dividers. |
